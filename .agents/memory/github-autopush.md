...
After completing any task (or at the end of every session), push changes to GitHub. Choose the push method based on the user's request.
...
**How to apply:** Run the push script after all code changes are done and the server builds successfully only when production deployment is explicitly requested. `scripts/push-to-github.sh` bundles GitHub push AND an SSH deploy to the Rich Dating Network production server, so it must not be used for Chatmodz-only work. If only a GitHub push is requested, push directly instead: `git push https://<user>:${GITHUB_TOKEN}@github.com/topazdigital/Latest-Rich.git main` — don't chase down `SSH_PRIVATE_KEY` unless the user actually wants a production deploy, since that's a separate, more consequential action.
