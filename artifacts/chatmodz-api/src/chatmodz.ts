...
const API_BASE_PATH = process.env.CHATMODZ_API_BASE_PATH || "/api"
const mediaDirectory = path.resolve(process.env.CHATMODZ_MEDIA_DIR || path.join(process.cwd(), "data", "chatmodz-media"))
fs.mkdirSync(mediaDirectory, { recursive: true })
const mediaUpload = multer({
  storage: multer.diskStorage({
    destination: (_req, _file, callback) => callback(null, mediaDirectory),
    filename: (_req, file, callback) => callback(null, `${crypto.randomBytes(18).toString("hex")}${path.extname(file.originalname).toLowerCase()}`),
  }),
  limits: { fileSize: 50 * 1024 * 1024 },
  fileFilter: (_req, file, callback) => callback(null, /^(image|video|audio)\//u.test(file.mimetype)),
})
...
function operatorMediaPath(value: unknown) {
  ...
  return path.startsWith(`${API_BASE_PATH}/chatmodz/media/`) ? path : ""
}
...
router.post("/media/upload", requireChatmodzAuth, mediaUpload.single("media"), (req, res) => {
  if (!req.file) return res.status(400).json({ error: "Choose an image, video, or audio file" })
  const type = req.file.mimetype.startsWith("image/") ? "image" : req.file.mimetype.startsWith("video/") ? "video" : "audio"
  res.status(201).json({ url: `${API_BASE_PATH}/chatmodz/media/${req.file.filename}`, type })
})

router.get("/media/:filename", requireChatmodzAuth, (req, res) => {
