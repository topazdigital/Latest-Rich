...
const configuredApiBase = (import.meta.env.VITE_CHATMODZ_API_BASE as string | undefined)?.replace(/\/+$/, "") || "";

/**
 * Keep the operator UI deployable beside the API or against a dedicated API
 * origin. Endpoints remain centralized so a deployment never needs a source
 * edit just to move the API.
 */
function apiUrl(path: string) {
  if (/^https?:\/\//u.test(path) || !configuredApiBase) return path;
  if (configuredApiBase.endsWith("/api/chatmodz") && path.startsWith("/api/chatmodz/")) {
    return `${configuredApiBase}${path.slice("/api/chatmodz".length)}`;
  }
  if (configuredApiBase.endsWith("/api") && path.startsWith("/api/")) {
    return `${configuredApiBase}${path.slice("/api".length)}`;
  }
...
function useAuthState(): AuthState & { login: (identifier: string, password: string) => Promise<void>; logout: () => void } {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl("/api/chatmodz/auth/me"), { credentials: "include", headers: { Authorization: `Bearer ${initial.token}` } })
  ...
  const login = async (identifier: string, password: string) => {
    const response = await fetch(apiUrl("/api/chatmodz/auth/login"), {
      method: "POST",
  ...
  const logout = () => {
    if (token) fetch(apiUrl("/api/chatmodz/auth/logout"), { method: "POST", credentials: "include", headers: { Authorization: `Bearer ${token}` } }).catch(() => undefined);
    localStorage.removeItem("chatmodz_auth");
    setUser(null);
    setToken(null);
  };
...
function photoUrl(photo?: string) {
  ...
  if (photo.startsWith("/")) return photo.startsWith("/api/") ? apiUrl(photo) : photo;
...
function ApplyPage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setError("");
    try {
      const response = await fetch(apiUrl("/api/chatmodz/applications"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
...
function useModeratorData() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!conversationResponse.ok || !statsResponse.ok) {
        throw new Error("The live operator queue is temporarily unavailable");
      }
      setConversations((await conversationResponse.json()).conversations || []);
      setStats(await statsResponse.json());
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "The live operator queue is temporarily unavailable");
    } finally {
      setLoading(false);
    }
  }, [token]);
  useEffect(() => { load(); const interval = window.setInterval(load, 15000); return () => window.clearInterval(interval); }, [load]);
  return { conversations, stats, loading, error, reload: load };
}

function QueuePage() {
  const { user } = useSession();
  const { conversations, stats, loading, error, reload } = useModeratorData();
  const [, setLocation] = useLocation();
  ...
  return <Shell><div className="page">
    ...
    <div className="metric-grid"><Metric label="Open conversations" value={String(stats.totalConversations)} detail={`${unread} waiting for a reply`} /><Metric label="Your sent replies" value={String(stats.messagesSent)} detail="Recorded by the live activity log" color="var(--teal)" /><Metric label="Active locks" value={String(stats.activeLocks)} detail="Locks expire after 10 minutes" color="var(--ink)" /><Metric label="Queue status" value={loading ? "…" : "Live"} detail="No demo records are shown" color="var(--teal)" /></div>
     <section className="panel"><div className="panel-head"><div><div className="panel-title">Anonymous conversations</div><div className="panel-kicker" style={{ marginTop: 5 }}>Operator view · site origin withheld</div></div><button className="button ghost compact" onClick={refresh}><RefreshCw size={13} /> Refresh</button></div>
       ...
       <div className="queue-list">{loading ? <div className="queue-skeleton" aria-label="Loading live conversations"><div className="skeleton-row"><span /><span /><i /></div><div className="skeleton-row"><span /><span /><i /></div><div className="skeleton-row"><span /><span /><i /></div></div> : error ? <div className="empty-state queue-error"><AlertTriangle size={27} /><strong>Queue connection interrupted</strong><span>{error}</span><button className="button primary compact" onClick={refresh}>Try again</button></div> : filtered.length ? filtered.map((conversation, index) => <ConversationRow key={conversation.key} conversation={conversation} userId={user?.id || 0} onOpen={() => setLocation(`/conversation/${conversation.key}`)} style={{ animationDelay: `${index * 35}ms` }} />) : <div className="empty-state"><Inbox size={27} /><strong>No conversations match this view</strong><span>The live source returned no matching conversations.</span></div>}</div>
...
function MediaBubble({ message }: { message: Message }) {
  ...
  const url = message.mediaUrl.startsWith("http") ? message.mediaUrl : message.mediaUrl.startsWith("/") ? (message.mediaUrl.startsWith("/api/") ? apiUrl(message.mediaUrl) : message.mediaUrl) : apiUrl(`/api/uploads/${message.mediaUrl}`);
  ...
  const send = async () => {
    ...
    try {
      ...
      if (media) {
        ...
        const upload = await authFetch(token, "/api/chatmodz/media/upload", { method: "POST", body: form });
...
function AuthenticatedRouter() {
  ...
  return <ErrorBoundary resetKey={location}><Switch><Route path="/" component={QueuePage} /><Route path="/welcome" component={LandingPage} /><Route path="/conversation/:id" component={ConversationPage} /><Route path="/reports" component={ReportsPage} /><Route path="/admin" component={AdminPage} /><Route path="/settings" component={SettingsPage} /><Route component={NotFound} /></Switch></ErrorBoundary>;
}

function App() {
  const auth = useAuthState();
  const routeBase = (import.meta.env.BASE_URL as string | undefined)?.replace(/\/$/, "") || "";
  return <QueryClientProvider client={queryClient}><AuthContext.Provider value={auth}><TooltipProvider><Router base={routeBase}><AuthenticatedRouter /><Toaster /></Router></TooltipProvider></AuthContext.Provider></QueryClientProvider>;
}
