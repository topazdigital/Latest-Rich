...
The frontend keeps API routing configurable for a dedicated deployment:

- `VITE_CHATMODZ_API_BASE` — optional API origin or path prefix. Leave unset
  when the API is served from the same origin; set it to the dedicated
  Chatmodz API origin for a separate deployment.
- `BASE_PATH` — Vite base path for the web client (use `/chatmodz/` for the
  hosted Chatmodz preview).
