...
import { Heart, Eye, EyeOff, Loader2, Chrome } from 'lucide-react'
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (window.google?.accounts) {
      ...
      if (script) script.onload = initGoogle
    }
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-2 group">
            <div className="w-12 h-12 rounded-2xl gradient-brand flex items-center justify-center shadow-lg group-hover:shadow-xl transition-shadow">
              <Heart className="w-6 h-6 text-white fill-white" />
            </div>
          </Link>
          <h1 className="text-2xl font-bold text-gray-900 mt-4">Welcome back</h1>
          <p className="text-gray-500 text-sm mt-1">Sign in to find your match</p>
        </div>
        ...
        <div className="card p-6 shadow-md">
          ...
          <p className="text-center text-sm text-gray-500 mt-5">
            No account?{' '}
            <Link href="/register" className="text-brand-500 hover:text-brand-600 font-semibold">Join Free ❤️</Link>
          </p>
        </div>
